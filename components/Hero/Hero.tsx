...
export default function Hero() {
  ...
  return (
    <section
      ref={containerRef}
      className="relative min-h-[100dvh] flex items-center justify-center overflow-hidden bg-(--color-background)"
    >
      {/* ===== Background Setup ===== */}
      <div className="absolute inset-0 z-0">
        <div className="relative w-full h-full parallax-bg">
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-(--color-background)/90 via-(--color-background)/60 to-transparent" />
          <div className="absolute inset-0 bg-gradient-to-t from-(--color-background) via-transparent to-transparent" />
        </div>
      </div>
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 lg:px-8 py-20 flex flex-col lg:flex-row items-center gap-16">
        ...
        <div ref={textRef} className="lg:w-1/2 space-y-10">
          ...
          <h1 className="hero-text-element opacity-0 text-5xl lg:text-7xl font-bold text-(--color-text-primary) leading-[1.1] tracking-tight">
            Discover <br />
            <span className="italic font-light text-(--color-secondary)">Luxury</span>{" "}
            Living at its Finest
          </h1>

          {/* Subtext */}
          <p className="hero-text-element opacity-0 text-lg text-(--color-text-secondary) max-w-xl leading-relaxed font-light">
            Experience the pinnacle of comfort and style. We curate exclusive properties that redefine your standard of living in the most sought-after locations.
          </p>

          {/* Search Bar - Modern Glassmorphism */}
          <div className="hero-text-element opacity-0 relative max-w-lg">
            <div className="absolute -inset-1 bg-gradient-to-r from-(--color-primary-light) to-(--color-secondary) rounded-2xl blur opacity-25" />
          ...
          {/* Stats Bar */}
          <div ref={statsRef} className="hero-text-element opacity-0 flex items-center gap-8 lg:gap-12 pt-8 border-t border-(--color-text-secondary)/10">
            <div className="hero-stats-item opacity-0">
              <p className="text-3xl font-bold text-(--color-text-primary)">2.5k+</p>
              <p className="text-sm text-(--color-text-secondary) uppercase tracking-wider mt-1">Properties</p>
            </div>
            <div className="w-px h-12 bg-(--color-text-secondary)/20" />
            <div className="hero-stats-item opacity-0">
              <p className="text-3xl font-bold text-(--color-text-primary)">98%</p>
              <p className="text-sm text-(--color-text-secondary) uppercase tracking-wider mt-1">Satisfaction</p>
            </div>
            <div className="w-px h-12 bg-(--color-text-secondary)/20" />
            <div className="hero-stats-item opacity-0">
              <p className="text-3xl font-bold text-(--color-text-primary)">10+</p>
              <p className="text-sm text-(--color-text-secondary) uppercase tracking-wider mt-1">Awards</p>
            </div>
          </div>
        </div>

        {/* ===== Featured Visuals (Right Side) - 3 Image Layout ===== */}
        <div ref={imageRef} className="lg:w-1/2 relative h-[600px] w-full hidden lg:block perspective-1000">
          ...
          <div className="hero-image-card opacity-0 absolute top-[10%] left-[15%] w-[65%] h-[80%] z-20 rounded-[2.5rem] overflow-hidden shadow-2xl border-4 border-(--color-background)">
            ...
            <div className="absolute bottom-6 left-6 right-6 backdrop-blur-md bg-white/10 border border-white/20 p-4 rounded-xl">
               <div className="flex items-center justify-between text-white">
                  <div>
                    <p className="text-xs uppercase tracking-wider opacity-80">Exclusive</p>
